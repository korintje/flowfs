...
pub async fn create_fileprops(
    ...
    // Prepare the query
    let rootdir: Vec<uuid::Uuid> = payload.fileprop_ids;
    let mut query = "INSERT INTO rootdir (name, file_url, completed) VALUES ".to_owned();
  
    // Prepare the values placeholder string and parameter values
    let mut params = Vec::new();
    for (i, fileprop) in rootdir.iter().enumerate() {
        if i > 0 {
            query.push_str(", ");
        }
