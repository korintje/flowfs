...
pub async fn list_fileprops(
    ...
    let cursor = match fileprops.aggregate(pipeline, options).await {
        Ok(cursor) => cursor,
        Err(e) => {
            error!("{}", e);
    ...
    let vc: Vec<FilePropRes> = vd.into_iter().map(|d| {from_document::<FilePropRes>(d).unwrap()}).collect();
    Ok(Json(FilePropsRes{fileprops: vc}))
}
...
pub async fn create_fileprop(
    ...
    let fileprops: Collection<FileProp> = db.collection("fileprops");
    let rt = match fileprops.insert_one(&payload, None).await {
        Err(e) => {
            error!("{}", e);
            return Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    ...
    if let Some(parent_id) = parent_id_wrap {
        match fileprops.update_one(
            doc! { "_id": parent_id },
            doc! { "$push": {"fileprops": inserted_id} },
            None,
        ).await {
            Err(e) => {
                error!("{}", e);
    ...
    match fileprops.find_one(doc!{"_id": inserted_id}, None).await {
        Err(e) => {
            error!("{}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
        Ok(None) => {
            error!("FileProp not found");
...
pub async fn show_fileprop(
    ...
    let options = AggregateOptions::builder().build();
    let mut cursor = match fileprops.aggregate(pipeline, options).await {
        Ok(cursor) => cursor,
        Err(e) => {
            error!("{}", e);
            return Err(StatusCode::INTERNAL_SERVER_ERROR)
    ...
    match cursor.try_next().await {
        Ok(Some(fileprop_res)) => Ok(Json(from_document(fileprop_res).unwrap())),
        Ok(None) => {
            error!("{}", "Cel not found");
            Err(StatusCode::NOT_FOUND)        
        },
...
pub async fn update_fileprop(
    ...
    */
    ...
    match fileprops.update_one(
        ...
        Ok(r) => {
            match fileprops.find_one(doc!{"_id": r.upserted_id}, None).await {
                Err(e) => {
                    error!("{}", e);
                    Err(StatusCode::INTERNAL_SERVER_ERROR)
                },
                ...
                Ok(Some(fileprop)) => Ok(Json(fileprop))
            }
        },
    }
}
